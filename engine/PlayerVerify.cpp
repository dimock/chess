...
void Player::verifyEscapeGenLimited(const Move & hmove, Figure::Type minimalType)
{
  EscapeGenerator eg(hmove, board_);
  EscapeGeneratorLimited egl(hmove, board_, minimalType, true);

  Move escapes[Board::MovesMax];
  Move limited[Board::MovesMax];
  int escN = 0, limN = 0;
  ...
  for ( ;; )
  {
    ...
    bool check = false;
    board_.makeMove(move);
    check = board_.underCheck();
    board_.unmakeMove();

    if ( board_.see(move) < 0 )
      continue;

    if ( !check && !move.capture_ )
      continue;

    if ( move.capture_ )
    {
      if ( board_.getField(move.to_).type() >= minimalType )
      {
        escapes[escN++] = move;
        continue;
      }
      else if ( board_.en_passant_ == move.to_ && board_.getField(move.from_).type() == Figure::TypePawn &&
        Figure::TypePawn >= minimalType )
      {
        escapes[escN++] = move;
        continue;
      }
  ...
  for (int i = 0; i < escN; ++i)
  {
    ...
    if ( !found )
    {
      char fen[256];
      board_.toFEN(fen);
      EscapeGeneratorLimited gel1(hmove, board_, minimalType, true);
      for ( ;; )
      {
        Move & move = gel1.next();
        if ( !move )
    ...
    THROW_IF( !found, "some move wasn't generated by limited escapes generator" );
  }
  ...
  for (int i = 0; i < limN; ++i)
  {
    ...
    THROW_IF( !found, "some extra move was generated by limited escapes generator" );
  }
}
#endif
...
void Player::verifyQuiesGenerator(const Move & hmove)
{
  ...
  MovesGenerator mg(board_);
  QuiesGenerator qg(hmove, board_, minimalType, 0);

  Move legal[Board::MovesMax], quies[Board::MovesMax];
  int n = 0, m = 0;
  ...
  for (int i = 0; i < n; ++i)
  {
    ...
    if ( !found )
    {
      char str[256];
      board_.toFEN(str);
      QuiesGenerator qg1(hmove, board_, minimalType, 0);
      while ( qg1.next() );
    }

    THROW_IF( !found, "move wasn't generated by quies generator" );
  }
  ...
  for (int i = 0; i < m; ++i)
  {
    ...
    THROW_IF( !found, "odd move was generated by quies generator" );
  }
  ...
  for (int i = 0; i < m; ++i)
  {
    for (int j = i+1; j < m; ++j)
    {
      ...
      THROW_IF( quies[i] == quies[j], "duplicate found in quies generator" );
    }
  }
}
#endif
